...
def show_navigation():
    ...
    col1,col2,col3,col4,col5=st.columns(5)
    col1.page_link("streamlit_app.py", label="Home", icon="🏠")
    col2.page_link(UI_DIR+"1_upload_files.py", label="Upload URL", icon="1️⃣")
    #col2.page_link(UI_DIR+"ui/2_transcribe_videos.py", label="Transcribe", icon="2️⃣")
    #col2.page_link(UI_DIR+"ui/3_qna.py", label="Q & A", icon="🌎")
    #col3.page_link(UI_DIR+"_fifx_data.py", label="Fix data")
    #col4.page_link(UI_DIR+"11_transcripts_with_answers.py", label="Transcript Q&A", icon="🌎")
    #col5.page_link(UI_DIR+"U_test_slack.py", label="Just testing")
    #col3.page_link(UI_DIR+"pages/chat_with_LMStudio.py", label="Chat LM Studio")
    #col4.page_link(UI_DIR+"pages/2_retreival_augmented_chat.py", label="RAG", icon="🌎")
    #cols=st.columns(len(navList)
    # col3.page_link("pages/1_chat_with_AI.py", label="Chat", icon="2️⃣", disabled=True)